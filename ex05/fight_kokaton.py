...
SCREENRECT = pg.Rect(0, 0, 1000, 664)
SCORE = 0
...
def main(winstyle=0):
    ...
    img = load_image("bom.png")
    Explosion.images = [img, pg.transform.flip(img, 0, 0)]
    Alien.images = [load_image(im) for im in ("teki.jpg", "teki.jpg", "teki.jpg")]
    Bomb.images = [load_image("egg.jpg")]
    Shot.images = [load_image("egg.jpg")]
    Ult.images = [load_image("ult.png")]
    ...
    while player.alive():
        ...
        if not player.reloading and firing and len(shots) < MAX_SHOTS:
            if not player.reloading and firing and len(shots)%10  == 0:  #画面の中で一発目なら
                Ult(player.gunpo())                                      #溜め攻撃を放つ
                if pg.mixer:
                    shoot_sound.play()
            else:
                Shot(player.gunpos())
